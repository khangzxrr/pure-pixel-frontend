...
import ShowcasesField, { type ShowcaseItem } from "./Component/ShowcasesField";
import type { PhotoshootPackageFormValues } from "./CreatePhotoshootPackage";

type UpdatePhotoshootPackageProps = {
  onClose: () => void;
};

export default function UpdatePhotoshootPackage({
  onClose,
}: UpdatePhotoshootPackageProps) {
  const {
  ...
  // the store starts with {} before a package is picked; the URLs below have always used its text form
  const packageId = String(selectedUpdatePhotoshootPackage);

  const { data: photoshootPackage } = useQuery({
    queryKey: [
      "package-detail-by-photographer",
      selectedUpdatePhotoshootPackage,
    ],
    queryFn: () => PhotoshootPackageApi.photographerFindById(packageId),
  });
  const { data: photoshootPackageShowcases } = useQuery({
    ...
    queryFn: () => PhotoshootPackageApi.getPhotoshootPackageShowcase(packageId),
  });
  // console.log(photoshootPackageShowcases && photoshootPackageShowcases);
  const { notificationApi } = useNotification();
  const [showcases, setShowcases] = useState<RcFile[]>([]);
  const [showcasesUrl, setShowcasesUrl] = useState<ShowcaseItem[]>(
  ...
  const updatePhotoshootPackage = useMutation({
    mutationFn: async (data: PhotoshootPackageFormValues) => {
    ...
    onSuccess: () => {
      ...
      setShowcases([]);
      setShowcasesUrl([]);
      // react-query v5 ignores plain string keys here, so both calls invalidate every query
  ...
  const isLoadingUpdatePhotoshootPackage = undefined;
