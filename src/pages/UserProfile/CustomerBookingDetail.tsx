...
type BookingPhoto = Schema<"SignedPhotoDto">;
// the detail also returns timestamps, which BookingDto does not declare
type BookingDetail = Schema<"BookingDto"> & {
  createdAt?: string;
  updatedAt?: string;
};
// reviews carry the reviewer id, which PhotoshootPackageReviewDto does not declare
type BookingReview = Schema<"PhotoshootPackageReviewDto"> & { userId?: string };
type NotificationEvent = { referenceType?: string };
...
const CustomerBookingDetail = () => {
  ...
  useEffect(() => {
    async function notificationBookingDetailEventHandler(
  ...
  const { data: bookingDetail, isPending } = useQuery({
    ...
    queryFn: (): Promise<BookingDetail> =>
      CustomerBookingApi.findById(String(bookingId)),
  });

  const { data: billItems } = useQuery({
    queryKey: ["customer-booking-bill-items", bookingId],
    queryFn: () => CustomerBookingApi.getBillItems(String(bookingId)),
  });

  const downloadPhoto = useMutation({
    mutationFn: (bookingId: string) =>
      CustomerBookingApi.downloadAllPhoto(bookingId),
  ...
  // Track the current index of the selected photo (-1 when there is none, as findIndex reports)
  const currentIndex =
    bookingDetail?.photos?.findIndex(
      (photo) => photo?.id === selectedPhoto?.id
    ) ?? -1;
  // Function to go to the previous photo
  const handlePreviousPhoto = () => {
    const photos = bookingDetail?.photos ?? [];
  ...
  const downloadAllPhoto = () => {
    ...
    downloadPhoto.mutate(String(bookingId)); // bookingId is passed to mutationFn
  };
  const currentDate = new Date(); // Get the current date and time
  // a missing timestamp gives an invalid date, as new Date(undefined) did
  const updatedAt = new Date(
    bookingDetail ? (bookingDetail.updatedAt ?? NaN) : 0
  ); // Convert updatedAt to a Date object
  // Convert updatedAt to a Date object and add 30 days
  const expiredAt = new Date(
    bookingDetail ? (bookingDetail.updatedAt ?? NaN) : Date.now()
  );
  ...
  const countTimeToDownload =
    updatedAt.getTime() - currentDate.getTime() + 30 * 24 * 60 * 60 * 1000; // Renderer callback with condition
  // console.log("Time difference in milliseconds:", countTimeToDownload);

  const renderer = ({ days, completed }: CountdownRenderProps) => {
    if (completed) {
  ...
  const scrollToPhoto = (photo: BookingPhoto | null) => {
  ...
  // the detail is also missing when loading failed, where every field below would be unreadable
  if (isPending || !bookingDetail) {
    return <div>Đang tải thông tin lịch hẹn...</div>;
  }

  const reviews: BookingReview[] | undefined = bookingDetail.reviews;
          ...
          <div className="flex flex-col gap-2 m-2 bg-[#2d2f34] rounded-lg">
            ...
            <div className="flex flex-col py-2 px-4 gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <ChatButton
                  userId={String(
                    bookingDetail.originalPhotoshootPackage?.user.id
                  )}
                />
                  ...
                  {bookingDetail.status === "SUCCESSED" && (
                    <p className="text-green-500">
                      ...
                      <span className="list-inside font-normal truncate">
                        {FormatDateTime(bookingDetail.updatedAt ?? NaN)}
                      </span>
                    </p>
                  )}
