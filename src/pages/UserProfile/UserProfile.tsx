...
// the page also shows counters that MeDto does not declare
type ProfileData = Schema<"MeDto"> & {
  _count?: { photos?: number; followers?: number; followings?: number };
};
...
const UserProfile = () => {
  ...
  const { data: userData, isLoading } = useQuery({
    ...
    // getMyProfile always loads the signed-in user; the route id only keys the cache
    queryFn: (): Promise<ProfileData> => UserProfileApi.getMyProfile(),
  });
  ...
  // Calculate container height based on scroll position (the parallax cover using it is commented out below)
  const _containerHeight = scrollY < 400 ? defaultHeight - scrollY * 0.5 : 0;
  useEffect(() => {
    const handleScroll = () => {
  ...
  if (isCurrentUpgradedPackageError)
    return <div>Error: {currentUpgradedPackageError?.message}</div>;
  ...
  function formatNumber(num: number | undefined) {
    ...
    } else if (num !== undefined && num >= 1000) {
