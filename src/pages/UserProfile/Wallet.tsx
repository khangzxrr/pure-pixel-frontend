...
export default function Wallet() {
  ...
  const sideNavDrawalRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const handleClickOutsideDrawal = (event: MouseEvent) => {
      if (
  ...
  const { data: wallet } = useQuery({
    // react-query v5 only takes array keys; the forms invalidate every query after a transaction
