...
const PhotoshootPackageManagementV2 = () => {
  ...
  const [orderByCreateAt] = useState<"asc" | "desc">("desc");
  ...
  const listPhotoshootPackages: PhotographerPhotoshootPackage[] =
    data?.objects || [];
  ...
  // the counts arrive as strings; arithmetic converts them the same way Number() does
  const usedPercentage = currentPackage
    ? (Number(packageCount) / Number(maxPackageCount)) * 100
    : 0;
          ...
          {parseInt(packageCount) < parseInt(maxPackageCount) ? (
            <div>
              <p className="text-sm font-normal m-2">
                ...
                {Math.floor(Number(maxPackageCount) - Number(packageCount))} gói
              </p>
