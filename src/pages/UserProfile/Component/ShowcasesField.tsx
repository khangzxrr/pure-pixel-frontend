import { Popconfirm, Tooltip, Upload, type UploadProps } from "antd";
...
// a saved showcase has an id; a photo added in this session only has its preview url
export type ShowcaseItem = { id?: string; photoUrl: string };

type ShowcasesFieldProps = {
  photoshootPackageId: string;
  showcases: RcFile[];
  setShowcases: Dispatch<SetStateAction<RcFile[]>>;
  showcasesUrl: ShowcaseItem[];
  setShowcasesUrl: Dispatch<SetStateAction<ShowcaseItem[]>>;
};
...
export default function ShowcasesField({
  ...
  const addPhotoshootPackageShowcase = useMutation({
    mutationFn: async ({
      ...
      photoshootPackageId: string;
      newShowcasePhoto: Blob | null;
    }) =>
      PhotoshootPackageApi.addPhotoshootPackageShowcase(photoshootPackageId, {
        newShowcasePhoto: newShowcasePhoto,
      }),
    onSuccess: () => {
      // react-query v5 ignores an array passed as the filters, so every query is invalidated
      queryClient.invalidateQueries();
    },
  });
  const { mutateAsync: addPhotoshootPackageShowcaseMutate } =
  ...
  const onShowcasesChange = async (info: UploadChangeParam<UploadFile>) => {
    try {
      ...
      const { originFileObj } = newFile;
      // console.log("newFile.status", newFile.status);
      // Add the new file to the current showcases list (files picked in the browser always carry originFileObj)
      if (newFile.status === "done" && originFileObj) {
        setShowcases((prevShowcases) => {
          const updatedShowcases = [...prevShowcases, originFileObj];
          return updatedShowcases;
        });

        // Generate the URL for the new file and add it to the current showcasesUrl
        const newUrl =
          await PhotoService.convertArrayBufferToObjectUrl(originFileObj);
  ...
  const deletePhotoFromShowcases = async (index: number) => {
  ...
  const customRequest: UploadProps["customRequest"] = async ({
    ...
    try {
      const response = await addPhotoshootPackageShowcaseMutate({
        photoshootPackageId: photoshootPackageId,
        // antd hands over the picked File here
        newShowcasePhoto: file instanceof Blob ? file : null,
      });
