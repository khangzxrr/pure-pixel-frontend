...
type ReviewBookingProps = {
  bookingId: string;
  // BookingDetailInfo passes "" when the booking has no review yet
  userReview?: Schema<"PhotoshootPackageReviewDto"> | "";
  role: string;
};
...
export default function ReviewBooking({
  ...
  const addReviewForBooking = useMutation({
    mutationFn: ({
      star,
      description,
    }: BodyOf<"CustomerBookingController_createReview">) =>
  ...
  const handleSubmit = () => {
    // validateFields already requires a star; checking it again narrows the type
    if (validateFields() && star) {
      // Submit logic here
      ...
      {userReview ? (
        <div className="flex flex-col gap-2 m-2 p-4 bg-[#2d2f34] rounded-lg">
          ...
          <p
            ...
            // React's <p> props have no placeholder; spread so the rendered markup stays the same
        ...
        <div className="flex flex-col gap-2 m-2 p-4 bg-[#2d2f34] rounded-lg">
          ...
          <div>
            <Rate
              ...
              // rc-rate draws a null value like 0
              value={star ?? 0}
            />
            {errors.star && (
