import { MessageCircle, Share2, User, Image, UserCheck } from "lucide-react";
...
// the header shows counters that PhotographerDTO does not declare
type PhotographerProfile = Schema<"PhotographerProfileDto"> & {
  photographer: Schema<"PhotographerDTO"> & {
    _count?: { followers?: number; followings?: number; photos?: number };
  };
};
...
const UserProfileV2 = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: (): Promise<PhotographerProfile> =>
      PhotographerApi.getPhotographerById(String(userId)),
  });
  const [hoveredIcon, setHoveredIcon] = useState<string | null>(null); // State để theo dõi icon nào đang được hover
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-4">
            <div className="flex items-end justify-between p-4">
              ...
              <div className="flex items-center space-x-4">
                ...
                {/* FollowButton reads the photographer, which is missing when the profile failed to load */}
                {data && <FollowButton photographer={data.photographer} />}
              </div>
            </div>
          </div>
        </div>
