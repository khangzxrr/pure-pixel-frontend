...
import UploadPhotoSell from "./components/UploadPhotoSell";
import UploadPhotoSellInfoBar, {
  type UploadPhotoSellFormHandle,
} from "./components/UploadPhotoSellInfoBar";
import MapBoxModal from "./components/MapBoxModal";
...
export default function SellPhoto() {
  ...
  const formRef = useRef<UploadPhotoSellFormHandle>(null);

  // lookups use the value as an object key, so String() finds exactly what the raw value did
  const photoData =
    getPhotoByUid(String(selectedPhoto)) !== undefined
      ? getPhotoByUid(String(selectedPhoto))
      : getPhotoByUid(String(uidHashmap[Object.keys(uidHashmap)[0]]));
