...
import {
  uploadPhotoSellInput,
  type UploadPhotoSellInputValues,
} from "../../../yup/UploadPhotoSellInput";
...
// what SellPhoto and UploadPhotoSell call through the form ref
export type UploadPhotoSellFormHandle = {
  submitForm: () => void;
  resetForm: () => void;
};

type UploadPhotoSellInfoBarProps = {
...
// kept as written; this antd version has no activeBorderColor Select token and ignores it
const selectTheme = {
...
// a photo is marked done right after its upload response is stored
type UploadedPhoto = SellPhotoItem & { response: Schema<"SignedPhotoDto"> };

// form fields reach the queue through updatePhotoPropertyByUid, so they are unknown here
...
const optionalStrings = (value: unknown) =>
  Array.isArray(value) && value.every((item) => typeof item === "string")
    ? value
    : undefined;
const optionalGps = (value: unknown): Schema<"GpsDto"> | undefined =>
  typeof value === "object" &&
  value !== null &&
  "latitude" in value &&
  "longitude" in value &&
...
// the default photo shape, with the queue's pricetags (width, height, price) as the price list
const toFormValues = (photo?: SellPhotoItem): UploadPhotoSellInputValues => ({
  ...getDefaultPhoto({
    ...photo,
    categoryIds: optionalStrings(photo?.categoryIds),
    photoTags: optionalStrings(photo?.photoTags),
    location: optionalString(photo?.location),
  }),
  photoTags: optionalStrings(photo?.photoTags) || [],
  pricetags: photo?.pricetags || [],
});

export default function UploadPhotoSellInfoBar({
  reference,
  selectedPhoto,
}: UploadPhotoSellInfoBarProps) {
  useImperativeHandle(reference, () => ({
    ...
    resetForm() {
      ...
      reset(toFormValues(selectedPhoto));
    },
  }));
  ...
  // the fields are only editable once a photo is shown, so this lookup always has a uid then
  const selectedUid = selectedPhoto?.file.uid ?? "";
  ...
  const [selectedLocate, setSelectedLocate] = useState<SelectedLocate>({
    latitude: selectedPhoto?.exif?.latitude ?? 10.762622,
    longitude: selectedPhoto?.exif?.longitude ?? 106.66667,
  });
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<UploadPhotoSellInputValues>({
  ...
  const updatePhotos = useMutation({
    ...
    mutationFn: async (photos: Parameters<typeof PhotoApi.updatePhotos>[0]) =>
      await PhotoApi.updatePhotos(photos),
  });
  const sellPhoto = useMutation({
    mutationFn: async (
      photos: Parameters<typeof PhotoExchange.sellPhotoByPhotoId>[0]
  ...
  const onSubmit = async () => {
    ...
    const photosToUpdate = photoArray.filter(
      (photo): photo is UploadedPhoto => photo.status === "done"
    );
    ...
    const updatePromises = photosToUpdate.map(async (photo) => {
      const photoToUpdate = {
        ...
        categoryId: optionalString(photo.categoryId),
        title: photo.title,
        description: photo.description,
        categoryIds: optionalStrings(photo.categoryIds),
        visibility: photo.visibility,
        photoTags: optionalStrings(photo.photoTags),
        gps: optionalGps(photo.gps),
      };
      const photoToSell = {
        id: photo.response.id,
        // pricetags arrive with the photo's resolutions; the submit button waits for a priced one
    ...
    try {
      ...
      // the original passed a bare key string, which v5 reads as "no filter": every query is invalidated
  ...
  useEffect(() => {
    ...
    if (
      ...
      setViewState((prev) => ({
        ...prev,
        latitude: Number(selectedPhoto.exif?.latitude),
        longitude: Number(selectedPhoto.exif?.longitude),
      }));
      setSelectedLocate({
        latitude: selectedPhoto.exif.latitude,
        longitude: selectedPhoto.exif.longitude,
            ...
            >
              ...
              <div>
                ...
                <Controller
                  ...
                  render={({ field }) => {
                    // the form holds the queue's pricetags, which also carry the size
                    const pricetags: { width?: number; height?: number; price?: number }[] =
                      Array.isArray(field.value) ? field.value : [];
                    // an empty fragment renders nothing, like the `false` this returned before
                    return !isDisableUpdatePhoto ? (
                        ...
                        {pricetags.map((_, index) => (
                          <div key={index}>
                            ...
                            >
                              ...
                              >
                                {pricetags[index]?.width || ""} x{" "}
                                {pricetags[index]?.height || ""}{" "}
                              </span>
                              ...
                              <NumericFormat
                                ...
                                onValueChange={(values) => {
                                  const updatedPricetags = [...(pricetags || [])];
