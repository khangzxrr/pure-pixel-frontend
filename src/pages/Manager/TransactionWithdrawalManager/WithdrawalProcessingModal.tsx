...
import { Popconfirm, Upload, type UploadProps } from "antd";
...
type AcceptWithdrawalVariables = { transactionId: string; photo: Blob };
type DenyWithdrawalVariables = { transactionId: string; failReason: string };

type WithdrawalProcessingModalProps = {
  selectedData: WithdrawalTransaction;
  tableRef: () => void;
  onClose: () => void;
};
...
export default function WithdrawalProcessingModal({
  ...
  tableRef,
  onClose,
}: WithdrawalProcessingModalProps) {
  const [isDeny, setIsDeny] = useState(false);
  const [detailReason, setDetailReason] = useState("");
  const [reason, setReason] = useState("");
  const [evidencePhoto, setEvidencePhoto] = useState<RcFile | null>(null); //evidence to proof that the system have been make transfer for withdrawal
  const [evidencePhotoUrl, setEvidencePhotoUrl] = useState("");
  // console.log(selectedData);
  const acceptWithdrawal = useMutation({
    mutationFn: ({ transactionId, photo }: AcceptWithdrawalVariables) =>
  ...
  const denyWithdrawal = useMutation({
    mutationFn: ({ transactionId, failReason }: DenyWithdrawalVariables) =>
  ...
  const onEvidencePhotoChange: UploadProps["onChange"] = async (info) => {
  ...
  const handleWithDrawal = async () => {
    ...
    // the confirm button stays disabled until a photo is picked
    if (!evidencePhoto) {
      return;
    }
    try {
