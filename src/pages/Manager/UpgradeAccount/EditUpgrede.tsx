...
type EditUpgredeProps = {
  selectedUpgrede: Partial<UpgradeRow>;
  onClose: () => void;
  tableRef: () => void;
};

export default function EditUpgrede({
  selectedUpgrede,
  onClose,
  tableRef,
}: EditUpgredeProps) {
  ...
  useEffect(() => {
    ...
    setValue(
      "maxPhotoQuota",
      Number(selectedUpgrede.maxPhotoQuota) / 1073741824,
    );
  }, [selectedUpgrede]);

  // descriptions is a list of plain strings, which typed field arrays do not support
  const methods = useForm<FieldValues>({
    resolver: yupResolver<FieldValues>(Upgrade),
    values: {
      ...selectedUpgrede,
      maxPhotoQuota: Number(selectedUpgrede.maxPhotoQuota) / 1073741824,
    },
  });
  const {
    handleSubmit,
  ...
  const onSubmit = (data: FieldValues) => {
    ...
    if (change !== null) {
      putData("/manager/upgrade-package", String(selectedUpgrede.id), {
        ...data,
        ...
        .catch((error: unknown) => {
          console.log(111, get(error, "response"));
          setDisabled(false);
          if (get(error, "data.statusCode") === 400) {
