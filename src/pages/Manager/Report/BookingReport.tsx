...
const statusRender = (status?: BookingStatus) => {
  switch (status) {
    case "REQUESTED":
...
export default function BookingReport({
  ...
  const { data: bookingDetail } = useQuery({
    ...
    queryFn: () =>
      ManagerReportApi.getBookingDetail(`${selectedData.referenceId}`),
  });
  const {
    user,
    ...
    totalBillItem,
    photos,
  }: Partial<Schema<"BookingDto">> = bookingDetail || {};
  ...
  // read apart from the branch below, which already narrowed reportType
  const isPhotographerReport =
  ...
  return (
    <div className=" text-gray-800 p-6 flex flex-col gap-3  max-w-3xl mx-auto">
                ...
                <>
                  ...
                  <div className=" flex gap-4 items-center mb-4">
                    ...
                    <div className="flex flex-col">
                      ...
                      <p className="text-gray-500">
                        {isPhotographerReport ? "Nhiếp ảnh gia" : "Khách hàng"}
                      </p>
                    </div>
                  </div>
                </>
      ...
      <div>
        ...
        {!photos ||
          // `!photos.length > 0`: only an empty list shows the message
          (photos.length === 0 && (
