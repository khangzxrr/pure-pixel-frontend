...
import { Upgrade, type UpgradeInput } from "../../../yup/Upgrade";

export type UpgradeReport = UpgradeInput & {
  id: string;
...
type EditReportProps = {
  // the tables never pass it (they give `selectedUpgrede`, which this form does not read)
  selectedReport?: UpgradeReport;
  onClose: () => void;
  tableRef: () => void;
};

// `error?.data?.statusCode`: Object() boxes primitives and turns null/undefined into {}
const statusCodeOf = (error: unknown): unknown =>
  Reflect.get(Object(Reflect.get(Object(error), "data")), "statusCode");
...
export default function EditReport({
  ...
  // untyped values: the form also holds fields the schema does not validate, and the
  // submit handler writes the parsed number back into the string price
  const methods = useForm<FieldValues>({
    resolver: yupResolver<FieldValues>(Upgrade),
  ...
  const onSubmit = (data: FieldValues) => {
    ...
    if (change !== null) {
      putData("/upgrade-package", String(selectedReport?.id), {
        ...data,
        status: "ENABLED",
      })
        .then(() => {
