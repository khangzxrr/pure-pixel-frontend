...
type DetailServicePackageProps = {
  selected: ServicePackageRow;
  reload: () => void;
};

// the server's error message, when the request reached it
...
export default function DetailServicePackage({
  ...
  const banPhoto = useMutation({
    ...
    onError: (error) => {
      notificationApi?.("error", "Khóa thất bạt", messageOf(error));
    },
  });
  ...
  const unBanPhotoshoot = useMutation({
    ...
    onError: (error) => {
      notificationApi?.("error", "Khóa thất bại", messageOf(error));
    },
  });
