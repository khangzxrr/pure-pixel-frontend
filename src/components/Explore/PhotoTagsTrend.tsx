...
const PhotoTagsTrend = () => {
  const [activeTag, setActiveTag] = useState<string | null>(null); // Trạng thái tag đang được chọn
  ...
  const {
    ...
    queryFn: (): Promise<ResponseOf<"PhotoTagController_getTopTags">> =>
      PhotoApi.getPhotoTags({ top: 5 }), // Gọi hàm API
  });
