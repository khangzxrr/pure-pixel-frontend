...
type ReportBody = BodyOf<"UserReportController_createReport">;
...
type ReportBookingModalProps = {
  visible: boolean;
  onClose: () => void;
  // the modal stays closed until an id is set
  id: string;
...
export default function ReportBookingModal({
  ...
  const reportByUser = useMutation({
    mutationFn: ({ content, reportType, referenceId }: ReportBody) =>
  ...
  const { mutate: reportByUserMutate } = reportByUser;
  const handleSubmit = () => {
