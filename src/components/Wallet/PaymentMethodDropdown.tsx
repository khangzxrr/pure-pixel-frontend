...
type PaymentMethodFilter = WalletTransactionFilter["paymentMethods"];

const getMethodLabel = (status: PaymentMethodFilter) => {
  switch (status) {
    case "":
...
type PaymentMethodDropdownProps = {
  ...
  setPaymentMethods: (paymentMethods: PaymentMethodFilter) => void;
...
export default function PaymentMethodDropdown({
  ...
  const filterMethod: MenuProps["items"] = [
    {
      key: "1",
      label: (
        <div
