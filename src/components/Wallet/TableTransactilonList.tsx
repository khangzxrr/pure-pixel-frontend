...
import { ConfigProvider, Pagination, type TableProps } from "antd";
...
import { WalletApi, type WalletTransactionFilter } from "../../apis/Wallet";
...
type TransactionDto = Schema<"TransactionDto">;

export default function TableTransactilonList() {
  const limit = 10;
  const [page, setPage] = useState(1);
  const [orderByCreatedAt, setOrderByCreatedAt] =
    useState<WalletTransactionFilter["orderByCreatedAt"]>("desc");
  const [types, setTypes] = useState<WalletTransactionFilter["types"]>("");
  const [statuses, setStatuses] =
    useState<WalletTransactionFilter["statuses"]>("");
  const [paymentMethods, setPaymentMethods] =
    useState<WalletTransactionFilter["paymentMethods"]>("");
  ...
  const {
    ...
    // keepPreviousData: true was removed in react-query v5 and has always been ignored
  });
  ...
  const columns: TableProps<TransactionDto>["columns"] = [
    {
      title: (
...

    {
      ...
      dataIndex: "updatedAt",
      key: "updatedAt",
      render: (updatedAt: TransactionDto["updatedAt"]) =>
