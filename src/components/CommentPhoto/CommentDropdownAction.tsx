...
export type CommentReference = {
  // sent by the backend but not declared on CommentDto
  photoId?: string;
  id: string;
};

type CommentDropdownActionProps = {
  handleEdit: () => void;
  photo: CommentReference;
};
...
const CommentDropdownAction = ({
  ...
  const deleteCommentPhoto = useMutation({
    mutationFn: ({ photoId, commentId }: DeleteCommentVariables) =>
      CommentApi.deleteComment(String(photoId), commentId), // Destructure the object
    onSuccess: () => {
