...
type CommentPhotoProps = {
  id: string;
  // accepted from the photo detail pages, never called
  reload?: () => void;
  top?: boolean;
};
...
export default function CommentPhoto({ id, top }: CommentPhotoProps) {
  ...
  const commentPhoto = useMutation({
    mutationFn: ({ id, content }: AddCommentVariables) =>
      CommentApi.addComment(id, content),
    onSuccess: () => {
