...
type UpgradeOrderInput = Parameters<typeof upgradePackageApi.upgradeOrder>[0];
...
export default function UpgradePaymentModal() {
  ...
  const { data: transactionDetail } = useQuery({
    ...
    queryFn: () =>
      TransactionApi.getTransactionById(
        selectedUpgradePackage.transactionId ?? "",
      ),
  ...
  const upgradePackage = useMutation({
    ...
    mutationFn: async (data: UpgradeOrderInput) =>
      await upgradePackageApi.upgradeOrder(data),
    onSuccess: (data) => {
      if (tranferType === "SEPAY") {
        ...
        setTimeout(() => {
          ...
          stopFireworks();
          // the string keys ("upgrade-package-list", "getTransactionById",
          // "check-migrate-package-fee") were read as empty filters, which invalidate every query
  ...
  const handleUpgrade = () => {
    // both confirm buttons only render once a payment method is chosen
    if (tranferType === "") return;
    setIsDisable(true);
    if (tranferType === "SEPAY") {
    ...
    upgradePackage.mutate({
      ...
      totalMonths: selectedUpgradePackage.minOrderMonth ?? 0,
    });
  };
  ...
  useEffect(() => {
    ...
    if (isUpgradePaymentModal && transactionDetail) {
      ...
      if (transactionDetail.status === "SUCCESS") {
        // the original invalidated { queryKey: "upgrade-package-list" }, { queryKey: "getTransactionById" }
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-gray-400 text-sm">
                        ...
                        {/* Reduced to text-sm */}
                        {(
                          Number(selectedUpgradePackage.maxPhotoQuota) /
                          1024 ** 3
                        ).toFixed(3)}{" "}
                        GB
                      </p>
