...
// the backend also sends these, the generated UpgradePackageDto does not declare them
export type UpgradePackageItem = Schema<"UpgradePackageDto"> & {
  descriptions?: string[];
  _count?: { upgradePackageHistories: number };
  totalMonths?: number;
};

// me/current-upgrade-package; the history row also carries originalUpgradePackageId
export type CurrentUpgradePackage = Schema<"UpgradeOrderDto"> & {
  upgradePackageHistory: { originalUpgradePackageId?: string };
};

type UpgradePackageCardProps = {
  packageItem: UpgradePackageItem;
...
const UpgradePackageCard = ({
  ...
  currentPackage,
  popularPackageId,
}: UpgradePackageCardProps) => {
  const { keycloak } = useKeycloak();
  const { setIsUpgradePaymentModal, setSelectedUpgradePackage } =
    useModalStore();
  ...
  return (
    <div className="flex flex-col gap-2 bg-[#292b2f] w-[400px] p-2 px-5 rounded-md">
      ...
      <div className="flex flex-col min-h-[250px] gap-2 mt-2">
        ...
        <div className="pb-2 border-b border-[#949494] text-sm font-normal">
          {(Number(packageItem.maxPhotoQuota) / 1024 ** 3).toFixed(3)} GB
        </div>
