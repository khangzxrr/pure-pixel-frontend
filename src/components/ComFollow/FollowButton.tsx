...
export type FollowablePhotographer = {
  id: string;
  isFollowed?: boolean;
};

type FollowButtonProps = {
  photographer: FollowablePhotographer;
  // accepted from callers but not used here
  userId?: string;
};

const FollowButton = ({ photographer }: FollowButtonProps) => {
  ...
  const followMutation = useMutation({
    mutationFn: (followingId: string) =>
      FollowApi.followPhotographer(followingId),
  });
