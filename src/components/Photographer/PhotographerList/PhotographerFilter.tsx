...
const PhotographerFilter = () => {
  ...
  const handleFilterByVote = (name: string, param: string) => {
          ...
          >
            <div className="py-1">
              {filterByPTGVote.map((item) => (
                // MenuItem renders its child and appended its className there; the class now sits on the child
                <MenuItem key={item.id}>
                  <div
                    onClick={() => handleFilterByVote(item.name, item.param)}
                    className="block px-4 py-2 text-sm text-[#eee] data-[focus]:bg-[#eee] data-[focus]:text-[#2f3136] transition-colors duration-200 ease-in-out hover:cursor-pointer"
                  >
                    {item.name}
                  </div>
                </MenuItem>
              ...
              <MenuItem>
                <div
                  ...
                  className="block px-4 py-2 text-sm text-red-500 data-[focus]:bg-red-500 data-[focus]:text-[#eee] transition-colors duration-200 ease-in-out hover:cursor-pointer"
                >
                  Xoá bộ lọc
                </div>
              </MenuItem>
