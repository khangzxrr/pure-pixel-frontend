...
type MapView = Coordinates & { zoom: number } & Partial<ViewState>;
type SearchBoxRetrieve = Parameters<
...
export default function MapBoxModal() {
  ...
  const [isLoadingCurrentLocation, setIsLoadingCurrentLocation] =
    useState(false);
  const [selectedLocate, setSelectedLocate] = useState<SelectedLocate | null>(
    null
  );
  ...
  // the store keys photos by uid; any other selection matches no photo, as before
  const selectedUid = String(selectedPhoto);
  ...
  const backToCurrentLocate = () => {
    ...
    // without a known location this read threw before the notice below could show
    if (
  ...
  const searchByCoordinate = useMutation({
    mutationFn: ({ longitude, latitude }: Coordinates) =>
      MapBoxApi.getAddressByCoordinate(longitude, latitude),
    onSuccess: (data) => {
      // the clicked point is stored before the address arrives
  ...
  const handleOk = async () => {
    if (selectedLocate) {
      ...
      updatePhotoPropertyByUid(selectedUid, "address", selectedLocate.title);
    }
  ...
  const handleRetrieve = (res: SearchBoxRetrieve) => {
