import { Checkbox, ConfigProvider, Input, Select, Tooltip } from "antd";
...
// the queue item carries the form fields under loosely typed keys
const toFormValues = (photo?: UploadPhotoItem): UploadPhotoInput => {
  const defaults = getDefaultPhoto({
    ...photo,
    exif: photo?.exif,
    categoryIds: Array.isArray(photo?.categoryIds) ? photo.categoryIds : undefined,
    photoTags: Array.isArray(photo?.photoTags) ? photo.photoTags : undefined,
  });
  return {
    ...defaults,
    photoTags: defaults.photoTags.filter(
      (tag): tag is string => typeof tag === "string"
    ),
    showExif: defaults.showExif === true,
  };
};
...
export default function UploadPhotoForm({
  ...
  const {
    ...
    defaultValues: toFormValues(selectedPhoto),
              ...
              <div className="relative w-full h-[40vh]">
                ...
                >
                  {selectedLocate && (
                    <>
                      <Marker
                        latitude={Number(selectedLocate.latitude)}
                        longitude={Number(selectedLocate.longitude)}
                        anchor="bottom"
                      >
                        <div
