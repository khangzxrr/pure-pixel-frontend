...
export default function PhotoCard({ photo }: PhotoCardProps) {
  ...
  const deletePhoto = useMutation({
    mutationFn: ({ id }: { id: string }) => PhotoApi.deletePhoto(id),
  });
  const { mutateAsync: deletePhotoMutate, isPending: isPendingDeletePhoto } =
    deletePhoto;
  const handleRemove = async (photo: UploadPhotoItem) => {
  ...
  const handleException = (errorMessage: unknown) => {
    switch (errorMessage) {
  ...
  const uploadPhoto = useMutation({
    ...
    onError: (e) => {
      console.log("uploadPhoto error", isAxiosError(e) ? e.response : undefined);
  ...
  const tryUploadPhoto = async ({ file }: { file: RcFile }) => {
  ...
  return (
    <div className="relative p-2">
      ...
      {photo.status === "uploading" && (
        ...
        >
          ...
          <p className="text-blue-500">
            {/* no progress reported yet reads as processing, as before */}
            {photo.percent !== undefined && photo.percent < 70
              ? "Đang tải ảnh lên"
              : "Đang xử lý ảnh"}
          </p>
        </div>
      )}
      {/* was `status !== "duplicated" || (status !== "invalid" && …)`, which only renders this for duplicated photos */}
      ...
      {/* was `status === "outQuota" || (status === "failed" && …)`, which only renders this for failed photos */}
      {photo.status === "failed" && (
          <div
            className={`absolute m-2 inset-0 grid place-items-center bg-red-300 bg-opacity-50 z-10 rounded-md cursor-pointer`}
            onClick={handleSelect}
          >
            {tryUploadPhotoPending ? (
              ...
              <Tooltip title="Thử lại" color="blue">
                {/* Tooltip forwarded its onClick to this child; the handler now sits here directly */}
                <div
                  className="flex flex-col items-center cursor-pointer text-white font-normal text-center hover:opacity-80"
                  onClick={(e) => {
                ...
                >
                  {/* <UndoOutlined className="text-4xl text-blue-500" /> */}
                  {/* only failed photos reach this overlay, so the outQuota text never showed */}
                  <p className="p-3  text-sm">Tải ảnh lên thất bại</p>
