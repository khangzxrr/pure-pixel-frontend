...
type UploadedPhoto = UploadPhotoItem & { response: Schema<"SignedPhotoDto"> };

// antd hands customRequest the RcFile it passed to beforeUpload
const isRcFile = (file: UploadRequestOptions["file"]): file is RcFile =>
  typeof file === "object" && "uid" in file;
...
export default function CustomUpload() {
  ...
  const uploadPhoto = useMutation({
    mutationFn: ({ file, onUploadProgress }: UploadVariables) =>
  ...
  const addWatermark = useMutation({
    mutationFn: async (photo: Parameters<typeof PhotoApi.addWatermark>[0]) =>
      await PhotoApi.addWatermark(photo),
  });

  //handle exception from api response
  const handleException = (file: RcFile, e: unknown) => {
  ...
  const beforeUpload = async (file: RcFile) => {
    ...
    // exif is null or false when parsing failed; reading Model from null threw
    if (!exif || !exif.Model) {
      notificationApi(
        "error",
        "Tải ảnh lên thất bại",
  ...
  const customRequest = async ({
    ...
    if (!isRcFile(file)) return;
    try {
      const response = await uploadPhoto.mutateAsync({
        file,
        onUploadProgress: (progressEvent) => {
          const percentCompleted = Math.round(
            (progressEvent.loaded / Number(progressEvent.total)) * 90
          );
  ...
  const handleChange = async (info: UploadChangeParam<UploadFile>) => {
    ...
    if (info.file.status === "error") {
      switch (
        isAxiosError<{ message?: string }>(info.file.error)
        ...
        return Promise.all([
          ...
          photo.watermark
            ? addWatermark.mutateAsync({
                ...
                text: String(photo.watermarkContent),
              })
            : Promise.resolve(),
        ]);
      });
        ...
        >
          ...
          >
            <Dragger
              ...
              // the queue items themselves: antd only matches them by uid (and assigns one when missing)
              fileList={photoArray as (UploadPhotoItem & UploadFile)[]}
              accept=".jpg,.jpeg,.png"
              style={{
                width: "100%",
                padding: "none",
