...
export default function ScrollingBar() {
  ...
  const photoRefs = useRef<Record<string, HTMLDivElement | null>>({}); // Object to store refs for each photo

  useEffect(() => {
    // only a uid selection has a card to scroll to
    if (typeof selectedPhoto === "string" && photoRefs.current[selectedPhoto]) {
