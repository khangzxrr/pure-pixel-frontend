...
type CameraPhotoProps = {
  nameCamera?: string;
};

type PublicPhoto = Schema<"SignedPhotoDto">;

const CameraPhoto = ({ nameCamera }: CameraPhotoProps) => {
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const limit = 20;
  const [selectedImage, setSelectedImage] = useState<PublicPhoto | null>(null);
  ...
  const handleOnClick = (photo: PublicPhoto) => {
