...
// the table reads photo and user counts, which CameraDto does not declare
type BrandCamera = Schema<"CameraDto"> & {
  _count?: { photos: number; cameraOnUsers: number };
};
...
const CameraByBrand = () => {
  ...
  // Thực hiện gọi API
  const { data, isLoading, isError, error } = useQuery<BrandCamera[]>({
    queryKey: ["camerasByBrand", brandId, topCamerasByBrand],
    // the route always has :cameraId
    queryFn: () =>
      CameraApi.getTopCamerasByBrandId(String(brandId), topCamerasByBrand),
  });
  ...
  const handleOnClickCamera = (name: string) => {
