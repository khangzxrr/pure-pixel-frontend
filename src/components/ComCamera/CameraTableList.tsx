...
import UseCameraStore, { type TopBrandCamera } from "../../states/UseCameraStore";
import type { Schema } from "../../apis/types";

// the backend sends maker.id, which MakerDto does not declare
type PopularBrand = Omit<Schema<"MakerWithUserCountDto">, "maker"> & {
  maker: Schema<"MakerDto"> & { id?: string };
};

// the store keys brands by maker id, so it always holds one
const toTopBrandCamera = (item: PopularBrand): TopBrandCamera => ({
  ...item,
  maker: { ...item.maker, id: item.maker.id ?? "" },
});
...
const CameraTableList = () => {
  ...
  const { data, isLoading, isError } = useQuery<PopularBrand[]>({
  ...
  useEffect(() => {
    if (data) {
      setListTopBrandCamera(data.map(toTopBrandCamera)); // Add the fetched array to Zustand store
    }
  }, [data, setListTopBrandCamera]);

  // the brand link passes only the brand, which cleared the camera name
  const handleOnClickCamera = (brand: string, name?: string) => {
          ...
          {isError && (
            <tr>
              <td colSpan={4} className="px-6 py-4 text-center text-red-500">
                {/* read isError.message (a boolean has none), so no detail was ever shown */}
                Error fetching data:{" "}
              </td>
            </tr>
          )}
