...
const BookingDetail = () => {
  ...
  useEffect(() => {
    ...
    if (bookingDetail?.photos && Array.isArray(bookingDetail.photos)) {
      for (const photo of bookingDetail.photos) {
    ...
    // the store actions are stable
  }, [bookingDetail, bookingId, clearState, addPhotoWithId, setSelectedPhotoByUid]);

  // a failed request left no booking to render (reading it threw)
  if (isPending || !bookingDetail) {
    return <div>Đang tải thông tin lịch hẹn...</div>;
  }
  // a photo object selection matches no uid, like before
  const selectedPhotoData =
    typeof selectedPhoto === "string" ? getPhotoByUid(selectedPhoto) : undefined;
