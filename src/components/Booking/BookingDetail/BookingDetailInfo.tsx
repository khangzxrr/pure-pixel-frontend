...
import {
  PhotoShootUpdateInput,
  type PhotoShootUpdateInputValues,
} from "../../../yup/PhotoShootUpdateInput";
import { FormatDate } from "../../../utils/FormatDate";
import type { BodyOf } from "../../../apis/types";
import type { BookingItem } from "../BookingRequestState/BookingCard";

type BookingUpdateInput = BodyOf<"PhotographerBookingController_updateBooking">;
...
const BookingDetailInfo = ({
  ...
  const {
    ...
    register,
    formState: { errors },
  } = useForm<PhotoShootUpdateInputValues>({
  ...
  const setBookingPaidMutation = useMutation({
    ...
    onSuccess: () => {
      // the string keys ("photographer-booking-detail", "get-all-photographer-booking")
      // were read as empty filters, which invalidate every query
  ...
  const updateBookingDetail = useMutation({
    mutationFn: ({ description, startDate, endDate }: BookingUpdateInput) => {
    ...
    onSuccess: () => {
      // ["booking-bill-items", id] was read as an empty filter, see above
      queryClient.invalidateQueries();
      setIsEdit(false);
    },
  });
  const handleOk = (data: PhotoShootUpdateInputValues) => {
    // console.log(data);
  ...
  // a missing updatedAt stays an invalid date, like before
  ...
  return (
    <div className="flex flex-col gap-1  w-full">
      ...
      {bookingDetail && bookingDetail.status === "SUCCESSED" && (
        <ReviewBooking
          // BookingDto has no bookingId; the photographer view never submits a review with it
          bookingId={String(bookingDetail.bookingId)}
