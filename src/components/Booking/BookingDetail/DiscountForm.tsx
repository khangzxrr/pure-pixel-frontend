...
// the price input holds the typed digits; yup turns them into the number that is submitted
type BillItemFormValues = Omit<BillItemInput, "price"> & {
  price: number | string;
};

// yupResolver types the form with the schema's output (a numeric price)
const billItemResolver = yupResolver(billItemSchema) as Resolver<BillItemFormValues>;

type DiscountFormProps = {
  bookingId: string;
};

export default function DiscountForm({ bookingId }: DiscountFormProps) {
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<BillItemFormValues, unknown, BillItemInput>({
    resolver: billItemResolver,
    defaultValues: {
      type: "DECREASE",
      title: "",
      price: "",
  ...
  const addBillItemMutation = useMutation({
    mutationFn: ({ title, description, price, type }: BillItemCreateInput) =>
  ...
  const handleAddBillItem = (data: BillItemInput) => {
  ...
  return (
    <div className="flex flex-col mt-4">
      <form onSubmit={handleSubmit(handleAddBillItem)}>
        <p className="font-normal mb-1 text-red-500">Giảm giá</p>
        <div className="grid grid-cols-1 md:grid-cols-10 gap-1">
          ...
          <div className="col-span-1 flex">
            <button
              type="submit"
              className="text-xs text-white hover:bg-opacity-80 px-2 py-1 rounded-md transition duration-100 bg-red-500"
            >
              Giảm
            </button>
          </div>
