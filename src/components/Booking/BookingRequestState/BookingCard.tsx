...
type BookingStatus = Schema<"BookingStatus">;

// timestamps and bookingId are read by the booking screens but not declared by BookingDto
export type BookingItem = Schema<"BookingDto"> & {
  createdAt?: string;
  updatedAt?: string;
  bookingId?: string;
};
...
type BookingCardProps = {
  booking: BookingItem;
  status: string;
  reportBooking: (bookingId: string) => void;
  // passed by BookingRequestList but not used here
  textStateColor?: string;
};

const BookingCard = ({ booking, status, reportBooking }: BookingCardProps) => {
  ...
  const acceptBookingMutation = useMutation({
    ...
    onSuccess: () => {
      // the string keys ("get-all-photographer-booking", "photographer-booking-detail")
  ...
  const denyBookingMutation = useMutation({
    ...
    onSuccess: () => {
      // "get-all-photographer-booking" and "photographer-booking-detail", see above
      queryClient.invalidateQueries();
      queryClient.invalidateQueries();
    },
  });
      ...
      >
        ...
        <div className="flex flex-col gap-1 px-4 py-2 hover:cursor-pointer">
          ...
          <div className="flex item-center gap-2 mt-2">
            ...
            <ChatButton userId={String(booking.user?.id)} />
