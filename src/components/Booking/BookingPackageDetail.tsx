...
type PhotoshootPackageInfoProps = {
  photoshootPackage: PhotoshootPackageItem;
  // parsed keycloak token of the signed-in user
  userData?: { sub?: string };
  onLogin: () => void;
};

const PhotoshootPackageInfo = ({
  photoshootPackage,
  userData,
  onLogin,
}: PhotoshootPackageInfoProps) => {
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 bg-[#292b2f] rounded-lg">
        ...
        <div className="flex flex-col gap-3 py-4 px-6">
          ...
          {userData ? (
            isBelongToCurrentUser ? (
              "" // Render nothing if `isBelongToCurrentUser` is true
            ) : (
              <button
                onClick={showModal}
