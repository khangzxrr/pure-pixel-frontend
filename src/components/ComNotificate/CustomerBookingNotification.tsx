...
type CustomerBookingNotificationProps = {
...
export default function CustomerBookingNotification({
  notification,
  onClose,
}: CustomerBookingNotificationProps) {
  const navigate = useNavigate();
  const bookingId = String(notification.payload?.id);
  ...
  const handleNavigate = (
    ...
    switch (referenceType) {
      ...
      case "CUSTOMER_BOOKING_PAID":
        navigate(`/profile/customer-booking/${bookingId}`);
        break;
