...
// "UPGRADE_PACKAGE"
export default function PhotoNotification({
  notification,
  onClose,
}: PhotoNotificationProps) {
  const navigate = useNavigate();
  const photoId = String(notification.payload?.id);
  const { data: photoDetail, error } = useQuery({
  ...
  //   photoDetail && photoDetail.photoSellings[0].active
  // );
  const handleNavigate = (referenceType: string) => {
