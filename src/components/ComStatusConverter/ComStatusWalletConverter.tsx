...
type TransactionStatus = Schema<"TransactionDto">["status"];

type ComStatusWalletConverterProps = {
  children?: TransactionStatus;
};

function ComStatusWalletConverter({ children }: ComStatusWalletConverterProps) {
  const convertStatus = (type?: TransactionStatus) => {
    switch (type) {
      case "SUCCESS":
