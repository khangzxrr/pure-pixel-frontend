...
type ViewFollowingsModalProps = {
  onClose: () => void;
};

const ViewFollowingsModal = ({ onClose }: ViewFollowingsModalProps) => {
  ...
  const { data, isLoading } = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => FollowApi.getAllFollowingMe(10, pageParam),
    initialPageParam: 0,
  ...
  const unFollowMutation = useMutation({
    mutationFn: (followId: string) => FollowApi.unFollow(followId),
  });
      ...
      >
        <div className="flex flex-col w-[600px] max-h-[600px]  min-h-[300px] relative gap-2">
          ...
          <div className="flex flex-col px-2 overflow-y-auto custom-scrollbar">
            ...
            {/* a failed request has no list; reading its length used to throw */}
            {!isLoading && listFollowings && listFollowings.length > 0 ? (
