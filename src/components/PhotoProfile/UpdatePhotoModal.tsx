...
import useModalStore, {
  type SelectedUpdatePhoto,
} from "../../states/UseModalStore";
import { CategoryApi } from "../../apis/CategoryApi";
import {
  updatePhotoInputSchema,
  type UpdatePhotoInput,
} from "../../yup/UpdatePhotoInput";
import getDefaultPhoto, { type DefaultPhoto } from "../../entities/DefaultPhoto";
...
// PATCH body: PhotoUpdateRequestDto plus the photo fields the form carries along
// (yup keeps keys its schema does not declare, e.g. id, categoryId, exif)
type UpdatePhotoBody = Parameters<typeof PhotoApi.updatePhotos>[0] &
  Omit<
    DefaultPhoto,
    | "categoryIds"
    | "photoTags"
...
// the visibility select only offers PUBLIC and PRIVATE
const toVisibility = (value: string): Schema<"PhotoVisibility"> | undefined =>
  value === "PUBLIC" || value === "PRIVATE" ? value : undefined;

// the form starts from the whole photo, so its other fields are submitted unchanged
const toFormValues = (photo: SelectedUpdatePhoto): UpdatePhotoInput => {
  const defaults = getDefaultPhoto(photo);
  return {
    ...defaults,
    photoTags: defaults.photoTags.filter(isString),
  };
};
...
export default function UpdatePhotoModal() {
  ...
  const [, setSelectedLocate] = useState<object>();
  ...
  const updatePhotos = useMutation({
    ...
    mutationFn: async (photos: UpdatePhotoBody) =>
    ...
    onSuccess: () => {
      ...
      // was invalidateQueries("my-photo"): a string is not a filter, so every query is invalidated
  ...
  const searchByCoordinate = useMutation({
    // photos without GPS have no coordinates; the lookup is still sent and fails
    mutationFn: ({
      longitude,
      latitude,
    }: {
      longitude?: number;
      latitude?: number;
    }) => MapBoxApi.getAddressByCoordinate(Number(longitude), Number(latitude)),
    onSuccess: (data) => {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<UpdatePhotoInput>({
    resolver: yupResolver(updatePhotoInputSchema),
    defaultValues: toFormValues(selectedUpdatePhoto),
  });
  ...
  const handleFinish = async (data: UpdatePhotoInput) => {
    ...
    const exif = selectedUpdatePhoto.exif;
  ...
  useEffect(() => {
    reset(toFormValues(selectedUpdatePhoto));
    getAllCategories.mutate();

    if (
      // "longitute" is a typo, so this is always true and the lookup always runs
