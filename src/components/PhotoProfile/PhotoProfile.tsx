...
// the counters come from a relation count MeDto does not declare
type MeProfile = Schema<"MeDto"> & {
  _count?: { photos?: number; followings?: number; followers?: number };
};

const PhotoProfile = ({ userData }: PhotoProfileProps) => {
  const [showFollowersModal, setFollowersShowModal] = useState(false);
  const [showFollowingsModal, setFollowingsShowModal] = useState(false);
  const { data, isLoading, isError, error } = useQuery<MeProfile>({
    queryKey: ["me"],
  ...
  // shows the profile query's error, which is empty when only this query failed
  ...
  // without a current package (204) there is no expiry date
