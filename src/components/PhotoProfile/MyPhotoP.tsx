...
type MyPhoto = Schema<"SignedPhotoDto">;

// the filter store keeps "" for "no order"; the API only takes asc/desc and skips anything else
const toSortOrder = (param: string): Schema<"SortOrder"> | null =>
  param === "asc" || param === "desc" ? param : null;

const MyPhotoP = ({ page, setPage, itemsPerPage }: MyPhotoPProps) => {
  ...
  const { data, isFetching, isError, error } = useQuery({
    ...
    queryFn: () =>
      PhotographerApi.getMyPhotos(
        itemsPerPage,
        page - 1,
        toSortOrder(orderByCreatedAt),
        toSortOrder(orderByUpVote),
        watermark,
        selling,
        searchResult,
        "RAW",
        // "" (not banned) sends no status filter
        banned || null
      ),
  });
  ...
  const handleOnClick = (photo: MyPhoto) => {
  ...
  // Parallax Effect State (the height it drove is commented out below, the scroll listener stays)
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 w-full bg-black bg-opacity-50 text-white text-center py-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 backdrop-blur-md">
                  <div className="flex justify-between px-1">
                    {photo.title && photo.title.length > 20
