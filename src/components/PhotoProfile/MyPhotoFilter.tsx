...
const MyPhotoFilter = () => {
  const { isWatermarkChecked, setIsWatermarkChecked, isBanned, setIsBanned } =
    UseMyPhotoFilter();
  const handleWatermarkChange = (event: ChangeEvent<HTMLInputElement>) => {
    const isChecked = event.target.checked;
    setIsWatermarkChecked(isChecked);
  };
  const handleForPhotoBanned = (event: ChangeEvent<HTMLInputElement>) => {
    const isChecked = event.target.checked;
  ...
  const handleFilterMyPhotoDate = (name: string, param: string) => {
    setFilterByPhotoDate(name, param);
  };
  const handleFilterMyUpVote = (name: string, param: string) => {
        ...
        <div className="flex flex-col  gap-2 ">
          <div className="flex items-center p-[5px] rounded-md">
            <label className="inline-flex items-center cursor-pointer gap-2">
              <span className="ms-3  font-normal text-[#eee] ">
              ...
              />
              <div className="relative w-11 h-6 bg-gray-200 peer-focus:outline-none  rounded-full peer dark:bg-[#202225] peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-[#6b7280]"></div>
            </label>
          </div>
          <div className="flex items-center p-[5px] rounded-md">
            <label className="inline-flex items-center gap-2 cursor-pointer">
              <span className="ms-3 text-[#eee] font-normal ">
                Bật/tắt ảnh bị cấm
              </span>
              <input
                id="forSaleCheckbox"
