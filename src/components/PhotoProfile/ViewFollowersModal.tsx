...
type ViewFollowersModalProps = {
  onClose: () => void;
};

const ViewFollowersModal = ({ onClose }: ViewFollowersModalProps) => {
  ...
  const { data, isLoading, isError, error } = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => FollowApi.getAllFolllowerMe(10, pageParam),
    initialPageParam: 0,
      ...
      >
        <div className="flex flex-col w-[300px] md:w-[600px] max-h-[600px]  min-h-[300px] relative gap-2">
          ...
          <div className="flex flex-col px-2 gap-2">
            ...
            {!isLoading && !isError && listFollowers && listFollowers.length > 0 ? (
              listFollowers.map((user) => (
                <div
                  key={user?.follower?.id}
                  className="flex items-center justify-between pb-2 mx-2 border-b border-[#8f8f8f]"
                >
                  <div
                    onClick={() => {
